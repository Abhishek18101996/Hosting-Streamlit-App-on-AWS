...
def readData(filename, fileindex, data_path):
    ...
    try:
        print('Trying to open model file')
        with FS.open(MODEL_PATH, 'rb') as data_file:
            print('Model file opened')

            f = h5py.File(data_file,'r')
            print('Model file converted to h5py')
            try:
                    print('trying to subcript the main file')
                    data = f['vil'][fileindex]
                    x1,x2,x3 = data[:,:,:13], data[:,:,13:26], data[:,:,26:39]
                    print('file subscripted')
            except Exception:
                    raise Exception(f'Data Error: {MODEL_PATH} File Corrupt. Check fileindex {fileindex}')
    ...
    try:
        if model_type == 'gan':
            ...
            try:
                ...
                raise Exception(e)
        	    #model = tf.keras.models.load_model(s3_object, compile=False, custom_objects = {"tf": tf})
        elif model_type == 'mse':    
